//-------------------------------------------------------------------------
//   Copyright 2002-2017 National Technology & Engineering Solutions of
...
CircuitBlock::CircuitBlock(
  ...
  Device::DeviceNameInstanceBlockMap &                          dNames,
...
bool CircuitBlock::parseNetlistFilePass1(PkgOptionsMgr &options_manager)
{
  ...
  std::vector< std::string > libInside(1,"");
...
void CircuitBlock::addTableData( DeviceBlock & device )
{
  ...
  std::pair<Device::DeviceNameInstanceBlockMap::iterator, bool> result = deviceNames_.insert(Device::DeviceNameInstanceBlockMap::value_type(dName, device.getDeviceData().getDevBlock()));
  if (!result.second) 
  {
    Report::UserError().at(device.getDeviceData().getDevBlock().getNetlistLocation()) << "Duplicate device "<< dName;
    (*result.first).second = Device::DeviceNameInstanceBlockMap::mapped_type(device.getDeviceData().getDevBlock());
  }

  if (netlistSave_)
  {
    (*result.first).second.numExtVars = device.getNumberOfNodes();

    if (DEBUG_IO) {
      int lastColon = dName.find_last_of( ':' );
      Xyce::dout() << "Inserting device: " << dName
                   << " locally named " <<  dName.substr( lastColon + 1 )
                   << " from file: " << (*result.first).second.getNetlistLocation().getFilename()
                   << " line: " << (*result.first).second.getNetlistLocation().getLineNumber() << std::endl;
    }

    topology_.addDevice(deviceManager_, device.getDeviceData(), &(*result.first).second);
  }
...
void CircuitBlock::addOptions(const Util::OptionBlock & options)
{
  ...
  if ( name == "PRINT" )
  {
    for (Util::ParamList::const_iterator it = options.begin(), end = options.end(); it != end; ++it)
      aliasNodeMapHelper_.insert(std::string((*it).uTag()));
  }
  ...
  std::vector< std::string > &  libInside)
{
  ...
  if (libSelect != libInside.back() && libInside.back() != "" && ES1 != ".ENDL")
  {
    return !eof;
  }
  ...
  else if (lineType == '.')
  {
    ...
    else if (ES1 == ".INCLUDE" || ES1 == ".INC" || ES1 == ".LIB")
    {
      ...
      Xyce::IO::handleIncludeLine( (ssfMap_.size()==1), netlistFilename_, line, 
    ...
    // added to support verilog-A begins
    else if (ES1 == ".HDL")
      {
        static std::string xyce_install_path=::getenv("XYCE_INSTALL_PATH");
	result = true;
	struct stat va_stat;
	int result_va;
    ...
    if ( !(line.empty()) )
    {
      ...
      if (lineType == 'L')
      {
        ...
        for( int i = 0; i < numMIs; ++i )
        {
          ...
          {
            device_.clear();
            device_.extractData(netlistFilename_, line, false);
